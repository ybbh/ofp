...
int main(int argc, char *argv[])
{
	...
	for (i = 0; i < num_workers; ++i) {
		...
		thr_params[i].arg = &pktio_thr_args[i];
	...
	odph_thread_common_param_init(&thr_common_param);
	thr_common_param.cpumask = &cpu_mask;
	thr_params[0].start = event_dispatcher;
	thr_params[0].arg = NULL;
	thr_params[0].thr_type = ODP_THREAD_WORKER;
	odph_thread_create(&dispatcher_thread,
			       &thr_common_param,
			       thr_params,
				   1);
