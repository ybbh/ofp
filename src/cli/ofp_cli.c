...
int ofp_start_cli_thread(odp_instance_t , int core_id, char *cli_file)
{
	odp_cpumask_t cpumask;
	struct ofp_global_config_mem *ofp_global_cfg;
	odph_thread_param_t thr_params;
	odph_thread_common_param_t common_param;
	...
	odph_thread_common_param_init(&common_param);
	common_param.cpumask = &cpumask;
	odph_thread_param_init(&thr_params);
	thr_params.start = cli_server;
	thr_params.arg = cli_file;
	thr_params.thr_type = ODP_THREAD_CONTROL;

	if (odph_thread_create(&ofp_global_cfg->cli_thread,
				   &common_param,
				   &thr_params,
				   1) == 0) {
